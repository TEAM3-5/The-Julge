 ...
 */
export type PostingItem = Omit<PostCardProps, 'className' | 'thumbnailClassName' | 'onClick'> & {
  id: string | number;
};
...
export function PostingList({
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        setVisibleCount((prev) => Math.min(prev + pageSize, posts.length));
      },
      {
        root, // 스크롤 기준이 되는 영역
  ...
  return (
    <section className={`flex flex-col gap-8 w-full ${className ?? ''}`}>
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-3 gap-x-3.5 gap-y-8">
