 ...
 */
export type PaginationButtonProps = PaginationNumberButtonProps | PaginationArrowButtonProps;
...
export const PaginationButton: React.FC<PaginationButtonProps> = (props) => {
  ...
  if (props.variant === 'number') {
    ...
    // 공통 스타일
    const base = 'inline-flex items-center justify-center w-10 h-10 rounded-[4px] tj-body2';
    // 현재 페이지(active) 여부에 따라 스타일 분기
    const classes = cn(
      base,
      isActive
        ? 'bg-primary text-white' // 선택 숫자 버튼
  ...
  const baseClasses = cn(
    ...
    disabled
      ? 'opacity-40 cursor-not-allowed' // 비활성 스타일
      : 'text-gray-black', // 활성 스타일
  );

  const IconSpan = <span className="inline-flex items-center justify-center w-5 h-5">{icon}</span>;
