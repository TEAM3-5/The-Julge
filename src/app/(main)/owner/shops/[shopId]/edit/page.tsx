...
import type { ShopUpdatePayload } from '@/types/shop';
...
// presigned URL 발급 + S3 업로드 (등록 페이지와 동일)
...
export default function ShopEditPage({ params }: ShopEditPageProps) {
  ...
  // shopId로 기존 가게 정보 불러와서 폼 초기값 세팅
  useEffect(() => {
    const fetchShop = async () => {
      try {
        ...
        showToast('가게 정보를 불러오지 못했습니다.', { variant: 'error' });
        router.back();
      } finally {
        setIsLoading(false);
      }
  ...
  if (isLoading) {
    return (
      <main className="flex h-full w-full items-center justify-center bg-gray-5">
        <p className="tj-body1">가게 정보를 불러오는 중입니다...</p>
      </main>
    );
  }
    ...
    <main className="flex w-full justify-center bg-gray-5">
      ...
      >
        <div className="flex items-center justify-between">
          <h1 className="tj-h1 text-gray-black">가게 정보 수정</h1>
          <Image
        ...
        <div className="flex flex-col gap-3">
          ...
          {imageUrl ? (
            <div
              className="relative h-[200px] md:h-[310px] w-full max-w-[483px] overflow-hidden rounded-xl border border-gray-30 bg-gray-10"
